...
# gosom/google-maps-scraper — primary discovery source (priority #1)
# github.com/gosom/google-maps-scraper
# Install: docker pull gosom/google-maps-scraper
try:
    from scrapers.gmaps_wrapper import disc_gosom, is_gosom_available
    _HAS_GOSOM = is_gosom_available()
except ImportError:
    _HAS_GOSOM = False
    def disc_gosom(niche, city, limit): return []       # noqa: E704
    def is_gosom_available(): return False               # noqa: E704
...
def _discover_combo(query: str, limit: int) -> list:
    """
    Try all discovery sources in priority order.
    Returns first non-empty result set.

    Priority:
      1. gosom/google-maps-scraper (Docker/binary) — most reliable, real rating+reviews
      2. Google Maps Playwright            — good data, needs browser sandbox
      3. Outscraper API                    — paid fallback, free credits on signup
      4. SerpApi Maps                      — 100 free/mo
      5. YellowPages HTML scraper          — free, good for service niches
      6. Nominatim (OSM)                   — always free, last resort
    """
    # Extract niche and city from query string for gosom
    # Query format: "roofing contractor in Atlanta, GA"
    niche, city = query, ""
    if " in " in query:
    ...
    # Priority 1: gosom (Docker or binary) — fixes the rating/reviews data gap
    if _HAS_GOSOM:
        r = disc_gosom(niche, city, limit)
        if r:
            print(f"    [gosom]   ✅ {len(r)} leads (rating+reviews populated)")
            return r
        print(f"    ↳ gosom returned 0 — trying Playwright")

    # Priority 2: Google Maps Playwright (best data when Docker unavailable)
    if _HAS_PLAYWRIGHT:
        r = _disc_google_maps_playwright(query, limit)
        if r: return r
        print(f"    ↳ Google Maps returned 0 — trying Outscraper")

    # Priority 3: Outscraper (paid but free credits on signup)
    if OUTSCRAPER_API_KEY:
        r = _disc_outscraper(query, limit)
        if r: return r
        print(f"    ↳ Outscraper returned 0 — trying SerpApi")

    # Priority 4: SerpApi (100 free/mo)
    if SERPAPI_KEY:
        r = _disc_serpapi(query, limit)
        if r: return r
        print(f"    ↳ SerpApi returned 0 — trying YellowPages")

    # Priority 5: YellowPages HTML scraper (free, no key, good for service niches)
    r = _disc_yellowpages(query, limit)
    if r: return r
    print(f"    ↳ YellowPages returned 0 — trying Nominatim")

    # Priority 6: Nominatim (always free, last resort)
...
def discover_leads() -> list:
    ...
    sources = []
    if _HAS_GOSOM:        sources.append("gosom/GMaps ✅ (PRIMARY)")
    if _HAS_PLAYWRIGHT:   sources.append("Playwright ✅")
...
# ══════════════════════════════════════════════════════════════════════════════
# ▌SECTION 6 — OUTPUT (Base44 push — Phase 2 rebuild)
# ▌All B44 bugs fixed: field mapping, upsert, PATCH, validation, decoupled paths
# ══════════════════════════════════════════════════════════════════════════════

# ── Wire base44_schema module into this file ──────────────────────────────────
# Import the schema module and inject the live API credentials into it so
# _b44_upsert, _sanitize_lead_payload, etc. all pick up the right keys.
try:
    import base44_schema as _b44s
    _b44s.BASE44_APP_ID      = BASE44_APP_ID
    _b44s.BASE44_API_KEY     = BASE44_API_KEY
    _b44s.BASE44_MAX_RETRIES = BASE44_MAX_RETRIES
    _b44s.FAILED_PUSH_LOG    = FAILED_PUSH_LOG
    _HAS_B44_SCHEMA = True
except ImportError:
    _HAS_B44_SCHEMA = False
    print("  ⚠️  base44_schema.py not found — Base44 push unavailable")


def _b44_sync_creds():
    """Re-sync credentials into base44_schema after config changes."""
    if _HAS_B44_SCHEMA:
        _b44s.BASE44_APP_ID      = BASE44_APP_ID
        _b44s.BASE44_API_KEY     = BASE44_API_KEY
        _b44s.BASE44_MAX_RETRIES = BASE44_MAX_RETRIES
...
def push_lead(lead: dict) -> str | None:
    """
    Push one lead to Base44 Lead entity.
    B44-5: GET by place_id first → PATCH if exists, POST if new.
    B44-8: validate before push.
    B44-1: field mapping + type fixes applied by _sanitize_lead_payload.
    B44-7: failure here does NOT affect push_wholesale_lead.

    Returns Base44 record id on success, None on failure.
    """
    if not _HAS_B44_SCHEMA:
        return None
    if not BASE44_APP_ID or not BASE44_API_KEY:
        print("      ⚠️  Base44 not configured — set BASE44_APP_ID + BASE44_API_KEY")
        return None

    # Absolute gate — disqualified leads never pushed
    if lead.get("disqualified") or lead.get("priority") == "disqualified":
        print(f"      ⏭  Skipping disqualified: {lead.get('disqualify_reason','')}")
        return None

    # Already pushed this run — return existing id
    ...
    # B44-8: build + validate payload before any HTTP call
    payload = _b44s._sanitize_lead_payload(lead)
    is_valid, errors = _b44s._validate_lead_payload(payload)
    if not is_valid:
        print(f"      ⚠️  [B44] Lead validation failed: {'; '.join(errors)}")
        _b44s._log_failed_push(lead, "Lead")
        return None

    # B44-5: upsert (GET → PATCH or POST)
    record_id, action = _b44s._b44_upsert("Lead", payload)

    if record_id:
        lead["lead_pushed"]    = True
        lead["base44_lead_id"] = record_id
        lead["crm_sync_status"] = "synced"
        _cp_save(lead)
        # B44-6: clear from failed log if this was a retry
        _b44s._clear_failed_push(lead.get("place_id",""), "Lead")
        print(f"      ✅ Lead {action} → id={record_id}")
        time.sleep(0.2)   # Base44 rate limit: stay under 5 req/s
        return record_id
...
def push_wholesale_lead(lead: dict, lead_id: str) -> str | None:
    """
    Push one lead to Base44 WholesaleLead entity.
    B44-7: fully decoupled from push_lead — failure/skip of one never
           affects the other.

    Returns Base44 wholesale record id on success, None on failure/skip.
    """
    if not _HAS_B44_SCHEMA:
        return None
    if not BASE44_APP_ID or not BASE44_API_KEY:
    ...
    if lead.get("disqualified") or lead.get("priority") == "disqualified":
        return None
    if lead.get("priority") == "cold":
        return None
    if (lead.get("composite_score") or 0) < MIN_SCORE_TO_PUSH:
        return None
    ...
    payload = _b44s._sanitize_wholesale_payload(lead)
    is_valid, errors = _b44s._validate_wholesale_payload(payload)
    if not is_valid:
        print(f"      ⚠️  [B44] Wholesale validation failed: {'; '.join(errors)}")
        _b44s._log_failed_push(lead, "WholesaleLead")
        return None

    record_id, action = _b44s._b44_upsert("WholesaleLead", payload)

    if record_id:
        lead["wholesale_pushed"]      = True
        lead["base44_wholesale_id"]   = record_id
        lead["listed_for_sale"]       = True
        _cp_save(lead)
        _b44s._clear_failed_push(lead.get("place_id",""), "WholesaleLead")
        print(f"      ✅ WholesaleLead {action} → id={record_id}")
        time.sleep(0.2)
        return record_id
...
def push_all(leads: list) -> dict:
    """
    Push all leads to Base44.
    B44-7: Lead push and WholesaleLead push are fully independent —
           a failed Lead push no longer skips the WholesaleLead push.
    """
    print("\n" + "═"*70)
    ...
    stats = {
        "lead_pushed": 0, "lead_failed": 0, "lead_skipped": 0,
        "wholesale_pushed": 0, "wholesale_skipped": 0, "wholesale_failed": 0,
    }

    for i, lead in enumerate(leads, 1):
        name  = lead.get("name", "?")
        score = lead.get("composite_score", 0)
        pri   = (lead.get("priority") or "?").upper()
        print(f"\n  [{i}/{len(leads)}] {name} | score={score} | {pri}")

        # ── Lead push (B44-7: result does NOT gate wholesale push) ────────────
        if lead.get("disqualified") or lead.get("priority") == "disqualified":
            print(f"      ⏭  Disqualified — skipping both entities")
        ...
        if lead_id:
            ...
            stats["lead_failed"] += 1
            # B44-7: do NOT continue — still attempt wholesale push below

        # ── Wholesale push (independent of lead push result) ─────────────────
        if lead.get("priority") in ("hot", "warm") and (lead.get("composite_score") or 0) >= MIN_SCORE_TO_PUSH:
            # Use the lead_id we got (could be None if lead push failed — wholesale
            # push builds its own payload independently)
            ws_id = push_wholesale_lead(lead, lead_id or "")
            if ws_id:
                stats["wholesale_pushed"] += 1
            else:
                stats["wholesale_failed"] += 1
        else:
            stats["wholesale_skipped"] += 1

        time.sleep(PUSH_THROTTLE)

    print(f"\n  Lead:      ✅ {stats['lead_pushed']} pushed  |  ❌ {stats['lead_failed']} failed  |  ⏭ {stats['lead_skipped']} skipped")
    print(f"  Wholesale: ✅ {stats['wholesale_pushed']} pushed  |  ❌ {stats['wholesale_failed']} failed  |  ⏭ {stats['wholesale_skipped']} skipped")
    total_fail = stats["lead_failed"] + stats["wholesale_failed"]
    if total_fail > 0:
        print(f"  ⚠️  {total_fail} failed push(es) → {FAILED_PUSH_LOG} — set RUN_MODE=\'push\' to retry")
    return stats


def retry_failed_pushes():
    """
    B44-6: Retry all entries in failed_pushes.json.
    On success, clears the entry from the log.
    Call this at start of push mode or after a failed run.
    """
    if not _HAS_B44_SCHEMA:
        return
    failed = _b44s.load_failed_pushes()
    if not failed:
        print("  ✅ No failed pushes to retry")
        return
    print(f"\n  Retrying {len(failed)} failed push(es)...")
    for entry in failed:
        pid    = entry.get("place_id")
        entity = entry.get("entity", "Lead")
        if not pid:
            continue
        try:
            lead = _cp_load(pid)
            # Reset push flags so push functions don't short-circuit
            if entity == "Lead":
                lead["lead_pushed"]    = False
                lead["base44_lead_id"] = None
                result = push_lead(lead)
                if result:
                    print(f"  ✅ Retry OK: {lead.get('name','?')} → {entity}")
            elif entity == "WholesaleLead":
                lead["wholesale_pushed"]    = False
                lead["base44_wholesale_id"] = None
                result = push_wholesale_lead(lead, lead.get("base44_lead_id",""))
                if result:
                    print(f"  ✅ Retry OK: {lead.get('name','?')} → {entity}")
        except FileNotFoundError:
            print(f"  ⚠️  Checkpoint not found for place_id={pid}")
        except Exception as e:
            print(f"  ⚠️  Retry error for {pid}: {e}")


def export_csv(leads: list):
    """
    D-6: Dynamic field list from lead dict keys.
    Excludes _homepage_html and all internal _ fields.
    """
    if not leads:
        print(f"  📄 CSV: {OUTPUT_CSV} (0 leads)")
        return
    # Build field list dynamically from actual lead keys, excluding internals
    exclude = {"_homepage_html"}
    all_keys = []
    seen_keys = set()
    ...
    priority_fields = [
        ...
        "owner_profile","recommended_package","package_price_setup","package_price_monthly",
        ...
        "groq_validated","data_completeness_score",
        "base44_lead_id","base44_wholesale_id","run_id","_discovery_source",
    ]
    for k in priority_fields:
        if k not in exclude and not k.startswith("_homepage"):
            all_keys.append(k)
            seen_keys.add(k)
    # Then add any remaining keys from actual leads
    for lead in leads:
        for k in lead.keys():
            if k not in seen_keys and k not in exclude and not k.startswith("_"):
                all_keys.append(k)
                seen_keys.add(k)
    ...
    with open(OUTPUT_CSV, "w", newline="", encoding="utf-8") as f:
        ...
        w.writeheader()
        for lead in leads:
            w.writerow({k: lead.get(k, "") for k in all_keys})
    print(f"  📄 CSV: {OUTPUT_CSV} ({len(leads)} leads, {len(all_keys)} fields)")
...
def print_summary(leads: list, stats: dict):
    ...
    disq = [l for l in leads if l.get("priority") == "disqualified"]
    sources = {}
    for l in leads:
        s = l.get("_discovery_source", "unknown")
        sources[s] = sources.get(s, 0) + 1
    avg_score = round(sum(l.get("composite_score",0) for l in leads) / len(leads)) if leads else 0
    avg_comp  = round(sum(l.get("data_completeness_score",0) for l in leads) / len(leads)) if leads else 0
    print("\n" + "█"*70)
    ...
    print(f"\n  Avg score           : {avg_score}")
    print(f"  Avg data completeness: {avg_comp}%")
    print(f"  Total LTV (hot+warm): ${sum(l.get('ltv_12mo_estimate',0) for l in hot+warm):,}")
    ...
    print(f"  Lead failed         : {stats.get('lead_failed', 0)}")
    print(f"  Wholesale pushed    : {stats.get('wholesale_pushed', 0)}")
    if hot:
        print(f"\n  TOP 5 HOT LEADS:")
...
if __name__ == "__main__":
    ...
    if RUN_MODE == "full":
        ...
        push_stats = push_all(all_leads)
        export_csv(all_leads)
        retry_failed_pushes()   # B44-6: clear successes from failed_pushes.json
